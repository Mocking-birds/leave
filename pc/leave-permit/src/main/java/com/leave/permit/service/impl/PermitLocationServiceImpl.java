package com.leave.permit.service.impl;

import java.util.List;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import com.leave.permit.mapper.PermitLocationMapper;
import com.leave.permit.domain.PermitLocation;
import com.leave.permit.service.IPermitLocationService;

/**
 * 位置信息Service业务层处理
 * 
 * @author admin
 * @date 2024-10-17
 */
@Service
public class PermitLocationServiceImpl implements IPermitLocationService 
{
    @Autowired
    private PermitLocationMapper permitLocationMapper;

    /**
     * 查询位置信息
     * 
     * @param locationId 位置信息主键
     * @return 位置信息
     */
    @Override
    public PermitLocation selectPermitLocationByLocationId(String locationId)
    {
        return permitLocationMapper.selectPermitLocationByLocationId(locationId);
    }

    /**
     * 查询位置信息列表
     * 
     * @param permitLocation 位置信息
     * @return 位置信息
     */
    @Override
    public List<PermitLocation> selectPermitLocationList(PermitLocation permitLocation)
    {
        return permitLocationMapper.selectPermitLocationList(permitLocation);
    }

    /**
     * 新增位置信息
     * 
     * @param permitLocation 位置信息
     * @return 结果
     */
    @Override
    public int insertPermitLocation(PermitLocation permitLocation)
    {
        return permitLocationMapper.insertPermitLocation(permitLocation);
    }

    /**
     * 修改位置信息
     * 
     * @param permitLocation 位置信息
     * @return 结果
     */
    @Override
    public int updatePermitLocation(PermitLocation permitLocation)
    {
        return permitLocationMapper.updatePermitLocation(permitLocation);
    }

    /**
     * 批量删除位置信息
     * 
     * @param locationIds 需要删除的位置信息主键
     * @return 结果
     */
    @Override
    public int deletePermitLocationByLocationIds(String[] locationIds)
    {
        return permitLocationMapper.deletePermitLocationByLocationIds(locationIds);
    }

    /**
     * 删除位置信息信息
     * 
     * @param locationId 位置信息主键
     * @return 结果
     */
    @Override
    public int deletePermitLocationByLocationId(String locationId)
    {
        return permitLocationMapper.deletePermitLocationByLocationId(locationId);
    }
}
